		...
		if request.method == 'GET':
			...
			try:
				...
				if help_filename!='':
					...
					filename=''
				members.update({"slug" : slugify(request.form['name']) , "name" : request.form['name'],'possition':request.form['possition'],'detail':request.form['detail'],'feature_image':filename,'facebook':request.form['facebook'],'linkin':request.form['linkin'],'duty':request.form['duty'] })
	...
	if action=='add':
		...
		if request.method == 'GET':
			...
			try:
				...
				#upload feature image
				if help=="":
					events.update({"slug" : slugify(request.form['title']) , "title" : request.form['title'],'description':request.form['description'],'date':request.form['date']})
				else:
					f.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
...
def contact(action=""):
	if request.method == 'POST':
		...
		return render_template("/wpm/contact.html")
			...
			if False:
				...
				if not slug:
					...
					#upload feature image
			   		if help_filename!="":
			   			# return filename
   						obj.update({"slug" : slugify(request.form['title']) , "title" : request.form['title'],'description':request.form['description'],"link" : request.form['link'],"category_id":request.form['category_id'],'feature_image':filename,'images':images})
   					else:
   						obj.update({"slug" : slugify(request.form['title']) , "title" : request.form['title'],'description':request.form['description'],"link" : request.form['link'],"category_id":request.form['category_id'],'images':images})
   					# return 'x'
   					status = db.session.commit()
	   				if not status:
...
def index(pagination=1):
	...
	review_top = Post.query.join(Category,Post.category_id == Category.id).filter(Category.slug=='blog').order_by(Post.id.desc()).limit(3)
	partner_active = Partner.query.order_by(Partner.id.desc()).limit(6).offset(0)
	partners = Partner.query.order_by(Partner.id.desc()).limit(6).offset(6)

	about = Page.query.filter_by(title='About')
	page1 = Page.query.filter_by(title='WOMAN PEACE MAKERS')
	page2 = Page.query.filter_by(title='WHO WE ARE')
	page3 = Page.query.filter_by(title='WHAT WE DO')
	page4 = Page.query.filter_by(title='GET INVOLVED')
	pages =[page1,page2,page3,page4]
	return render_template(template+'/index.html',pages = pages,about=about,mem=members,partners=partners,partner_active=partner_active,review_top=review_top,reviews=reviews,form=form,page_name='home',posts_top=posts_top,home_posts=home_posts,posts_bottom = posts_bottom,pagin=int(pagin),current_pagin=int(pagination),posts=posts);
...
def single(slug='',pagination=1):
	if slug=='resources':
		...
		if(math.ceil(Post.query.join(Category,Post.category_id == Category.id).filter(Category.slug=='news').count())%limit != 0 ):
			pagin=int(pagin+1)

		return render_template(template+'/news.html',page_name='category',category_slug='news',category_name='News',posts=posts,pagin=int(pagin),current_pagin=int(pagination))
	elif slug=='blog':
		posts=Post.query.join(Category,Post.category_id == Category.id).filter(Category.slug=='blog').order_by(Post.id.desc()).limit(limit).offset(int(int(int(pagination)-1)*limit))
		pagin=math.ceil((Post.query.join(Category,Post.category_id == Category.id).filter(Category.slug=='blog').count())/limit)
		if(math.ceil(Post.query.join(Category,Post.category_id == Category.id).filter(Category.slug=='blog').count())%limit != 0 ):
			pagin=int(pagin+1)

		return render_template(template+'/blog.html',page_name='category',category_slug='blog',category_name='blog',posts=posts,pagin=int(pagin),current_pagin=int(pagination))
	...
	if slug=='get-involve':
		...
		page3 = Page.query.filter_by(slug='donate-us')
		page2 = Page.query.filter_by(slug='volunteer')
		...
		mission = Page.query.filter_by(slug='mission')
		corevalue = Page.query.filter_by(slug='core-value')
		whoweare = Page.query.filter_by(slug='who-we-are')
		members= Member.query.all()
	...
	try:
		...
		if post_object.count()>0:
			...
			pages=[]
			if slug=='who-we-are':
				page1 = Page.query.filter_by(slug='our-organizational-pledge')
				page2 = Page.query.filter_by(slug='our-vision')
				page3 = Page.query.filter_by(slug='our-mission')
				page4 = Page.query.filter_by(slug='partners')
				page5 = Page.query.filter_by(slug='get-involved')
				page6 = Page.query.filter_by(slug='our-values')
				page7 = Page.query.filter_by(slug='our-story')
				pages =[page1,page2,page3,page4,page5,page6,page7]
			projects=[]
			if slug=='what-we-do':
				page8 = Page.query.filter_by(slug='knowledge-seeking')
				page2 = Page.query.filter_by(slug='training')
				page3 = Page.query.filter_by(slug='mobilization')
				page4 = Page.query.filter_by(slug='our-work')
				page5 = Page.query.filter_by(slug='our-thematic-expertise')
				post1 = Post.query.join(Category,Post.category_id == Category.id).filter(Post.slug=='gender-womens-issues-program')
				post2 = Post.query.join(Category,Post.category_id == Category.id).filter(Post.slug=='peace-conflict-transformation-program')
				post3 = Post.query.join(Category,Post.category_id == Category.id).filter(Post.slug=='gender-and-peace-leadership-programs')
				page6 = Page.query.filter_by(slug='our-approaches')
				projects=[page8,page2,page3,page4,page5,post1,page6,post2,post3]
				# members=Member.query.order_by(Member.id.desc()).all()
				# projects = Post.query.join(Category,Post.category_id == Category.id).filter(Category.slug=='our-projects').order_by(Post.id.desc())
			return render_template(template+"/page.html",projects=projects,pages=pages,partners=partners,page_name="page",page_object=page_object,members=members)
		else:
	...
	if slug == "gender-womens-issues-program" or slug == "peace-conflict-transformation-program" or slug == "gender-and-peace-leadership-programs" :
		return render_template(template+'/single3B.html',events=events,form=form,page_name='single',related_posts=related_posts,post_object=post_object)
	# if post_object.first().category.slug=='blog':
	# 	return render_template (template+'/singleBlog.html',events=events,form=form,page_name='single',related_posts=related_posts,post_object=post_object)
	else:
